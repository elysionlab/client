...
BOOL Dlg_Setup::OnInitDialog()
{
	...
	m_pCtrl_Path.SetWindowText(m_pMainWindow->m_rPathBasic);
...
bool Dlg_Setup::Skin_ButtonPressed(CString m_ButtonName)
{
	...
	else if(m_ButtonName == "BTN_SEARCH")
	{
		...
		CSelectFolderDialog pSelectDialog(FALSE, m_pMainWindow->m_rPathBasic, wFlag, NULL, this, "기본으로 저장될 폴더를 선택해 주세요");
	...
	else if(m_ButtonName == "BTN_OK")
	{
		...
		m_pCtrl_Path.GetWindowText(m_pMainWindow->m_rPathBasic);
