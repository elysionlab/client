...
{	
	...
	//20080528 오픈 자료실 추가, jyh
	if(!ACE_OS::strcmp(pArgv, "OpenUpDownServer"))
		RunServerInfo.nServerPORT = SERVER_PORT_OPEN;
...
int ConfigUpdate()
{
	...
	if(!InitDefault)
	{
		...
		*/
		...
		{
			idx++;
			sprintf_s(szTemp, 50, "DATABASE_IP%d", idx);
...
bool GetHostIP()
{
	...
	if( gethostname ( name, sizeof(name)) == 0)
	{
		if((hostinfo = gethostbyname(name)) != NULL)
		{
			...
			{		
				// 사설아이피 인지 체크
				lstrcpy(RunServerInfo.pServerIp, inet_ntoa(*(struct in_addr *)hostinfo->h_addr_list[i]));
