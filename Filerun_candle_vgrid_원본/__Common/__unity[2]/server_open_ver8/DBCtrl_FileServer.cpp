...
bool DBCtrl::CheckService()
{
	...
	try
	{
		...
		sprintf_s(m_pQuery1, 4096, "Select ftp From %s Where flag_use = 'Y' And flag_state = 'Y'", TABLE_COMMON_FTP_SERVER);	
...
int DBCtrl::LoadFileCountCore(char* pSaveName)
{
	...
	try
	{
		...
		sprintf_s(m_pQuery1, 4096, "Select count(*) From %s Where filename='%s'", TABLE_COMMON_FILES, pSaveName);
...
int DBCtrl::LoadFileCountCp(char* pCheckSum, __int64 nFileSize)
{
	...
	int			nCountRef = 0;
	// 참조카운트
	sprintf_s(m_pQuery2, 4096, "Select Count(*) From %s Where md5sum='%s' and size=%I64d and contents_complete = 1 " \
...
__int64 DBCtrl::LoadNewFileCheck(char* pSaveName, __int64 nFileSize, __int64 nVolumeIdx, bool bTrick)
{
	...
	try
	{
		...
		sprintf_s(m_pQuery1, 4096, "Select no From %s Where temp_name = '%s' and temp_volume = %I64d and size = %I64d", TABLE_FILE_BBS, pSaveName, nVolume, nFileSize);
		if((pResult = pDataBase->ExecuteQueryR(m_pQuery1, nCount)) == 0 || (pRow = mysql_fetch_row(pResult)) == 0)
		{
			SAFE_DELETEDB(pResult);	
			return -1;
...
int DBCtrl::SetNewFileComplete(char* pSaveName, __int64 nFileSize, __int64 nVolume)
{
	...
	try
	{
		...
		sprintf_s(m_pQuery1, 4096, "Insert Into %s (ftp_volume, filename, regdate) Values(%I64d, '%s', UNIX_TIMESTAMP())",  TABLE_COMMON_FILES, nVolume, pSaveName);
		pDataBase->ExecuteQuery(m_pQuery1);
		/*
...
__int64 DBCtrl::LoadFtpAllotment()
{
	...
	try
	{
		...
		sprintf_s(m_pQuery1, 4096, "Select f_volume.volume From %s f_volume, %s f_server Where " \
							"f_volume.ftp = f_server.ftp And f_volume.flag_upload = 'Y' " \
							"And f_server.flag_state = 'Y' And f_server.flag_use = 'Y' " \
							"Order By f_server.loadavg Asc, f_volume.freesize Desc limit 1",  
...
bool DBCtrl::LoadFtpAllotmentQueue(__int64 nVolume)
{
	...
	try
	{
		...
		sprintf_s(m_pQuery1, 4096, "Select f_volume.freesize From %s f_volume, %s f_server Where " \
...
bool DBCtrl::LoadFtpInfoUpload(ACE_UINT64 nVolumeIdx, ACE_UINT16 nSpeed, char* pServerIP, int &nServerPORT, char* pServerID, char* pServerPW)
{
	...
	try
	{
		...
		sprintf_s(m_pQuery1, 4096, "Select f_server.port, f_server.ip, f_account.passwd, f_account.id, f_server.flag_state, f_server.flag_use " \
		...
		lstrcpy(pServerIP, pRow[1]);
		lstrcpy(pServerPW, pRow[2]);
		lstrcpy(pServerID, pRow[3]);
		lstrcpy(pFlag_Use, pRow[4]);
		lstrcpy(pFlag_State, pRow[5]);
		
		SAFE_DELETEDB(pResult);

		//printf("ftp정보 - nTempVolume: %I64d, pServerIP: %s, pServerPT: %d, pServerID: %s, pServerPW: %s, pFlag_Use: %s, pFlag_State: %s\n", 
		//	nVolumeIdx, pServerIP, nServerPORT, pServerID, pServerPW, pFlag_Use, pFlag_State);
...
//	MYSQL_ROW	pRow;
	...
	try
	{
		...
		// 볼륨의 상태 조회
		sprintf_s(m_pQuery2, 4096, "Select f_server.flag_state, f_server.flag_use " \
...
bool DBCtrl::LoadFtpCheck(ACE_UINT64 nVolumeIdx)
{
	...
	try
	{
		...
		sprintf_s(m_pQuery1, 4096, "Select f_server.loadavg, sum(f_account.count_up), sum(f_account.count_down) " \
							"From %s f_volume, %s f_account, %s f_server Where " \
							"f_volume.volume = %I64d And f_volume.ftp = f_server.ftp And " \
...
void DBCtrl::DownCountSet_Criterion(char *pSaveName)
{
	...
	sprintf_s(m_pQuery1, 4096, "Update %s Set count_criterion = count_criterion + 1 Where filename = '%s'", TABLE_COMMON_FILES, pSaveName);
	pDataBase->ExecuteQuery(m_pQuery1);
}
