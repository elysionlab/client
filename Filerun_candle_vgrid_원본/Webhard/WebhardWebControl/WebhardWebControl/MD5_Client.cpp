...
CString CMD5::GetString(CString sFileName, DWORD &dwErrorCode)
{
	MD5_CTX context;
	unsigned char digest[16];
	unsigned int len = (unsigned int)sFileName.GetLength();
	

	CFile pFileData;	
	BYTE pBuffer[MAX_BUFFER_SIZE];
	...
	try
	{
		...
		if(!pFileData.Open(sFileName,  CFile::modeRead | CFile::shareDenyNone))
		{
		...
		{
			dwBytesRead = pFileData.Read(pBuffer, sizeof(pBuffer));

			while(dwBytesRead > 0)
			{
				nFileSize += dwBytesRead;

				MD5Update(&context, (unsigned char*)pBuffer, dwBytesRead);
		...
		for(int i = 0; i < 16; i++)
		{
			sTemp.Format("%02x", digest[i]);
			strResult += sTemp;
		}
	}
	catch(...)
	{
		::MessageBox(NULL, "File Exception : MD5", "Error", MB_OK);
	}
		
	return strResult;
}

void CMD5::MD5Init(MD5_CTX* context)                    /* context */
{
