...
int DBCtrl::AuthUser(mmsv_User* pClient, ACE_Message_Block* pDataBlock)
{
	...
	try
	{
		...
		// 아이디 존재 유무 체크 : CP 체크
		sprintf(m_pQuery1, "Select idx From %s Where %s = '%s' And state != 0", 
		...
		// 나머지 정보 가져오기
		/*sprintf(m_pQuery1,  "Select %s, state, UNIX_TIMESTAMP() From %s Where %s = '%s'",  
							FIELD_MEMBER_PW, TABLE_MEMBER_BASE, FIELD_MEMBER_ID, pLogin->pUserID);*/
		//20081022 회원 스피드값 추가, jyh
		sprintf(m_pQuery1,  "Select %s, state, UNIX_TIMESTAMP(), speed From %s Where %s = '%s'",  
			FIELD_MEMBER_PW, TABLE_MEMBER_BASE, FIELD_MEMBER_ID, pLogin->pUserID);
		...
		// 정지회원체크
		//printf("정지회원 체크-> pRow[1]:%s, pRow[2]:%s\n", pRow[1], pRow[2]);//test
		...
		pLogin->nSpeed = atoi(pRow[3]);		//20081022 스피드 값
...
int DBCtrl::Fix_Check(mmsv_User* pClient, char* pUserID)
{
	...
	try
	{
		sprintf(m_pQuery1,  "Select fix_end, UNIX_TIMESTAMP() From %s Where userid = '%s'",  TABLE_MEMBER_PAY, pUserID);
		if((pResult = ExecuteQueryR(m_pQuery1, nCount)) == 0 || (pRow = mysql_fetch_row(pResult)) == 0)
		{			
			SAFE_DELETEDB(pResult);				
			return 0; // 에러
