			...
			if(RunServerInfo.nServerRunMode == MMSV_SERVER_MODE_UP)
			{
				switch(pHead->code)
				{
					...
					*/
					...
					//case LOAD_MUREKA:
					//{
					//	pDataBase->UpLoadMureka_BBS(pClient, pDataBlock);
					//	return 0;
					//}
					...
					{
						pDataBase->UpLoadState_Copyright_data(pClient, pDataBlock);
						return 0;
					}
			...
			else if(RunServerInfo.nServerRunMode == MMSV_SERVER_MODE_DOWN)
			{
				switch(pHead->code)
				{
					...
					*/
					...
					// 업로드전 저작권 체크값 수신
					case DOWN_SAVE_MUREKA_DATA:
					{
						pDataBase->DownLoadState_Copyright_data(pClient, pDataBlock);
						return 0;
					}
