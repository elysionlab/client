...
{	
	...
	InitNodeLib("filerun-0000-0000"); //브이그리드
	SetDownMainSvrInfo(strSIP,62120,5);//플렉스 넷, IP/POrt

	//AfxMessageBox(_T(strSIP));
	...
	//AfxMessageBox(_T((m_pMainWindow->m_pPtrTrans->pFolderName)));
	//AfxMessageBox(_T(m_strTargetPath));
	//AfxMessageBox(_T((m_pMainWindow->m_pPtrTrans->pFileName)));
	//AfxMessageBox(_T(m_pMainWindow->m_pPtrTrans->nFileSizeReal));
	//AfxMessageBox(_T(file_md5));
