...
BOOL NetworkController::MMSocketConnectTCP()
{
	...
	for(int nIndex = 0; nIndex < 3; nIndex++)
	{
		...
		if(nRet == SOCKET_ERROR)
		{
			...
			CString errorcode;
			errorcode.Format("SOCKET_ERROR 서버 연결 실패!! 에러코드: %d", error);
			AfxMessageBox(errorcode);//20071122
			m_bConnectServer = false;
	...
	{
		AfxMessageBox("Thread Pool 초기화 실패!");//20071122
		return FALSE;
	}	
	else
	{
...
{	
	...
	//AfxMessageBox("송신 스레드 생성됨!!");//20071122
	__try		
	{
		m_bThreadSend = true;
...
{	
	...
	//AfxMessageBox("수신 스레드 생성!!");//20071122
	__try		
	{
		// 쓰레드 시작
		m_bThreadRecv = false;
...
BOOL NetworkController::MMRecvPost()
{
	__try
	{
		...
		if(m_Sock == INVALID_SOCKET)
		{
			...
			sprintf_s(strtemp, MAX_PATH, "INVALID_SOCKET: %d", m_Sock);
			//AfxMessageBox(strtemp);//20071122
			return false;
		}
		...
		if(SOCKET_ERROR == ret)
		{
			...
			if(ErrCode != WSA_IO_PENDING)
			{	//AfxMessageBox("WSARecv() 실패!");//20071122
				return FALSE;
			}
		}
		return TRUE;
