...
void mmsv_User::force_close(const mmsv_User* pUserPtr)
{
	...
	if(((ACE_OS::gettimeofday() - m_tAccessTime).sec()) < CONNECT_LIMIT_TIME)
	{
		...
		sprintf_s(pSendPtrB, 1024, "%s::%d", pUserPtr->m_pAddr_Remote.get_host_addr(), (int)pUserPtr->m_pAddr_Remote.get_port_number());
...
{
	sprintf_s(m_pUserInfo, 255, "{MEMBERIDX=%I64d}:{USERID=%s}:{IP=%s::%d}:{TIME=%d}",
