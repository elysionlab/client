	...
	try
	{
		...
		if(lstrcmp(upHash->szIndex, ""))
		{
			...
			if(cnt == pTokenIdx->GetCount())
			{
				...
				sprintf(m_pQuery2, "Delete From %s where link_idx = %d", TABLE_HASH_COPYRIGHT, upHash->link_idx);
				//printf("%s\n", m_pQuery2);
				nRet = ExecuteQuery(m_pQuery2);
				//hash_bbs 테이블에서도 삭제
				sprintf(m_pQuery2, "Delete From %s where idx = %d", TABLE_HASH_BBS, upHash->link_idx);
				nRet = ExecuteQuery(m_pQuery2);
			...
			if(upHash->nResult != UPLOAD_RESULT_HASH_REG_EXIST)
			{
				...
				{
					sprintf(m_pQuery2, "Update %s Set md5sum = '%s' where idx = %d", TABLE_HASH_COPYRIGHT, pTokenCheckSum->GetToken(j), atoi(pTokenIdx->GetToken(j)));
					//printf("%d: %s\n", j, m_pQuery2);
					nRet = ExecuteQuery(m_pQuery2);
		...
		else
		{
			for(size_t i=0; i<pTokenCheckSum->GetCount(); i++)
			{
				...
				else
				{
					...
					sprintf(m_pQuery2, "Update %s SET state = 99 where idx = %d", TABLE_BBS, upHash->link_idx);
					nRet = ExecuteQuery(m_pQuery2);
...
void DBCtrl::UpLoadGuraEnd_BBS(mmsv_User* pClient, ACE_Message_Block* pDataBlock)
{
	...
	try
	{
		...
		sprintf(m_pQuery1, "Update %s Set complete = 1 where idx = %d", TABLE_HASH_COPYRIGHT, pGuraUploadEnd->nFileIdx);
		...
		{
			sprintf(m_pQuery1, "Update %s Set state = 1 where idx = %d LIMIT 1", TABLE_HASH_BBS, pGuraUploadEnd->nBoardIndex);
			nRet = ExecuteQuery(m_pQuery1);
		}
...
void DBCtrl::UpLoadItem_BBS(mmsv_User* pClient, ACE_Message_Block* pDataBlock, unsigned int nPS)
{
	...
	try
	{
		...
		{			
			...
			sprintf(m_pQuery1,  "Select t_file.no, t_file.size, t_file.realname, t_path.localname " \
