...
BOOL CRegistry :: Connect( LPCTSTR pszRemote_p, HKEY hKey_p ) {
	...
	if( pszRemote )
	   strcpy_s( pszRemote, strlen(pszRemote_p)+1, pszRemote_p );
	...
	for( register DWORD i = 0 ; /*i < dwSubKeyCnt*/ ; ++i ) {
		...
		if( lRet == ERROR_SUCCESS ) {
			...
			if( lRet == ERROR_SUCCESS ) {
				...
				strcpy_s(pszPath, strlen(pszSubKey_p)+1, pszSubKey_p);
				strcat_s(pszPath, strlen(pszPath), TEXT("\\"));
				strcat_s(pszPath, strlen(pszPath), pszKeyName);
...
void CRegVal :: SetValue( LPCTSTR pszValue_p, BOOL bExpanded_p ) {
	const DWORD dwSize = (DWORD)(pszValue_p ? (_tcslen(pszValue_p) * sizeof(TCHAR)) : 0);
...
void CRegVal :: SetValue( LPCTSTR pszValue_p [], const DWORD nArray_p ) {
	...
	for( i = 0 ; i < nArray_p ; ++i )
		nSize += (DWORD)((_tcslen( pszValue_p[i] ) * sizeof(TCHAR)) + sizeof(TCHAR));
...
void CRegVal :: SetValue( const CStringArray & rArray_p ) {
	...
	register int i ;
	DWORD nSize = 0 ;
	const int nElem = (int)rArray_p.GetSize();
